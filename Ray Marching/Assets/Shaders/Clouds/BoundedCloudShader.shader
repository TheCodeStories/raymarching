...
//         // float computeTransmittance(float3 position, float3 Ld, float Ldist)
//         // {
//         //     float T = 1;
//         //     int steps = min(100, int(Ldist/(_ShadowStepSize+0.01)));
//         //     for(int i=0;i<steps;i++){
//         //         float3 sp = position + Ld*(i*_ShadowStepSize);
//         //         float d = sampleDensity(sp);
//         //         T *= exp(-pow(d,_ExponentialFactor));//*_ShadowStepSize);
//         //         if(T<0.01) break;
//         //     }
//         //     return T;
//         // }
...
//         float computeTransmittance(float3 position, float3 Ld, float Ldist)
//         {
//             float tau = 0; // accumulated optical depth
//             int steps = max(1, min(100, int(Ldist/(_ShadowStepSize+0.001))));
//             for (int i = 0; i < steps; i++) {
//                 float3 sp = position + Ld * (i * _ShadowStepSize);
//                 float d = sampleDensity(sp);
//                 tau += pow(d, _ExponentialFactor) * _ShadowStepSize;
...
//                     float3 Ld  = normalize(_LightPosition - position);
//                     float  Ldist = length(_LightPosition - position);
//                     float  Tsh = computeTransmittance(position, Ld, Ldist);
